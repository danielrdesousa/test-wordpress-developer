...
<div id="comments" class="<?php echo comments_open() ? 'comments-area' : 'comments-area comments-closed'; ?>">
	<div class="<?php echo $discussion->responses > 0 ? 'comments-title-wrap' : 'comments-title-wrap no-responses'; ?>">
		...
		if ( comments_open() ) {
			if ( have_comments() ) {
				_e( 'Join the Conversation', 'twentynineteen-child' );
			} else {
				_e( 'Leave a experience', 'twentynineteen-child' );
			}
		} else {
			if ( '1' == $discussion->responses ) {
				/* translators: %s: Post title. */
				printf( _x( 'One reply on &ldquo;%s&rdquo;', 'comments title', 'twentynineteen-child' ), get_the_title() );
			} else {
				printf(
	...
	if ( have_comments() ) :
		...
		if ( have_comments() ) :
			...
			$comments_text = __( 'Experiences', 'twentynineteen' );
		...
		if ( comments_open() && 'asc' === strtolower( get_option( 'comment_order', 'asc' ) ) ) :
			...
			<div class="comment-form-flex">
				<span class="screen-reader-text"><?php _e( 'Leave a experience', 'twentynineteen-child' ); ?></span>
				<?php twentynineteen_comment_form( 'asc' ); ?>
				<h2 class="comments-title" aria-hidden="true"><?php _e( 'Leave a experience', 'twentynineteen-child' ); ?></h2>
			</div>
			<?php
		endif;
